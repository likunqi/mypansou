...
async function getSiteConfig() {
  ...
  return {
    ...
    // 自定义代码块（多条，每条可配注入位置/生效范围/启停）
    site_custom_blocks: parseArr(cfg.site_custom_blocks),
...
async function saveSiteConfig(obj) {
  ...
  ["site_notices", "site_ads", "site_custom_blocks"].forEach(function (k) {
...
module.exports = {
  ...
  migrateFromJson,
  // 采集词库 + 豆瓣资料（8-07 新增，MySQL 权威，直接透传）
  keywordCrawlerList: mysql.keywordCrawlerList, keywordCrawlerAdd: mysql.keywordCrawlerAdd, keywordCrawlerImport: mysql.keywordCrawlerImport,
  keywordCrawlerUpdate: mysql.keywordCrawlerUpdate, keywordCrawlerDelete: mysql.keywordCrawlerDelete, keywordCrawlerBatchSet: mysql.keywordCrawlerBatchSet,
  keywordCrawlerStats: mysql.keywordCrawlerStats, keywordCrawlerTake: mysql.keywordCrawlerTake, keywordCrawlerMarkCrawled: mysql.keywordCrawlerMarkCrawled,
  doubanList: mysql.doubanList, doubanGet: mysql.doubanGet, doubanUpsert: mysql.doubanUpsert, doubanUpdate: mysql.doubanUpdate,
  doubanDelete: mysql.doubanDelete, doubanStats: mysql.doubanStats, doubanLinkResources: mysql.doubanLinkResources,
};
