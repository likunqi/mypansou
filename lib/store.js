...
async function keywordRecord(kw) {
  ...
  else arr.push({ keyword: kw, search_count: 1, is_hot: 0, sort_order: 0, status: 1 });
...
async function keywordEnsure(kw, fields) {
  ...
  if (!Array.isArray(arr)) arr = [];
  var hit = arr.find(function (x) { return String(x.keyword) === String(kw); });
  if (hit) { hit.is_hot = fields.is_hot ? 1 : 0; hit.sort_order = fields.sort_order || 0; hit.status = fields.status === undefined ? 1 : fields.status; }
  else arr.push({ keyword: kw, search_count: 0, is_hot: fields.is_hot ? 1 : 0, sort_order: fields.sort_order || 0, status: fields.status === undefined ? 1 : fields.status });
...
module.exports = {
  ...
  keywordRecord, keywordEnsure, keywordList, keywordUpdate, keywordDelete,
  hotResources, aiSummaryAdd, aiSummaryList,
  migrateFromJson,
};
