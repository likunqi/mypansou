...
function deleteFiles(fids, cookie) {
  // 夸克删除接口：body 必须 filelist（对象数组 [{fid:...}]），传 fid_list 字符串数组会报 "filelist 不能同时为空"
  var filelist = (fids || []).map(function (fid) { return { fid: String(fid) }; });
  return api('POST', '/1/clouddrive/file/delete?pr=ucpro&fr=pc&__t=' + Date.now(), cookie, { filelist: filelist })
    .then(function(r) {
