...
function deleteFiles(fids, cookie) {
  // 参考实现（_quark_ref/quark_auto_save.py delete）：
  //   payload = {"action_type": 2, "filelist": <字符串数组>, "exclude_fids": []}
  //   querystring 带 uc_param_str=""
  // 缺少 action_type/exclude_fids 或 filelist 用对象数组都会导致 inner error / Bad Parameter
  return api('POST', '/1/clouddrive/file/delete?pr=ucpro&fr=pc&uc_param_str=&__t=' + Date.now(), cookie, {
    action_type: 2,
    filelist: (fids || []).map(String),
    exclude_fids: []
  })
    .then(function(r) {
      if (r.data && r.data.status === 200) { return r.data.data || { deleted: true }; }
      throw new Error('delete failed: ' + (r.data ? r.data.message : ''));
    });
}
...
module.exports = { transfer, getShareInfo, saveFiles, createShare, parseUrl, ensureDir, listDir, deleteFiles, queryTask };
