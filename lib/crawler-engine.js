...
// ---------- 主流程 ----------
// XML 实体解码：RSSHub 的 item description 里 <img> 是 &lt;img&gt; 转义形式，
// 不解码则 regex 规则（如 <img[^>]+src="...">）永远匹配不上，thumbnail 提取为 0
  ...
  try {
    for (var p = page; p <= pageEnd; p++) {
      ...
      for (var i = 0; i < items.length; i++) {
        ...
        // 实体解码：让 <img>/<enclosure> 等标签可被 regex 规则匹配（thumbnail 提取关键）
        if (itemText) itemText = decodeEntities(itemText);
        var rec = {};
        var ok = true;
        ...
        // 描述字段名兼容：规则表历史字段是 desc，入库字段是 description（曾因不一致导致描述全部丢弃）
        if (!rec.description && rec.desc) rec.description = rec.desc;
        rec.disk_type = rec.disk_type || source.disk_type || "quark";
        // 归一化网盘类型：阿里云盘采集/API 会返回 alipan，统一为 aliyun（前端/筛选/统计都按 aliyun）
        if (rec.disk_type === "alipan") rec.disk_type = "aliyun";
