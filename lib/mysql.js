...
async function ensureSeed() {
  ...
  // 豆瓣热词采集定时任务（每日 08:00，幂等）
  try {
    await execute(
      "INSERT IGNORE INTO scheduled_tasks (task_name, task_type, interval_sec, task_config, status) " +
      "VALUES ('豆瓣热词采集', 'douban_hotwords', 86400, JSON_OBJECT('time','08:00','top',10), 1)");
  } catch (e) { console.error("[mysql] 豆瓣热词任务种子失败:", e.message); }
  // 首页热门推荐缓存预热（每 30 分钟，幂等）
  try {
    await execute(
      "INSERT IGNORE INTO scheduled_tasks (task_name, task_type, interval_sec, task_config, status) " +
      "VALUES ('热门推荐预热', 'trending_prewarm', 1800, JSON_OBJECT(), 1)");
  } catch (e) { console.error("[mysql] 热门推荐预热任务种子失败:", e.message); }
}
...
module.exports = {
  ...
  transferTop, resourceStats, aiSummaryAdd, aiSummaryList, ensureTable,
