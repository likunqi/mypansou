...
// 配置格式：{"pansou": true} 旧布尔 或 {"pansou": {"enabled": true, "disks": ["quark","baidu"]}} 新对象（disks 空数组=不限制）
...
function parseSourceCfg(v, defEnabled) {
  ...
  return { enabled: v === undefined ? !!defEnabled : !!v, disks: [] };
}
...
// 后台管理列表：全量源信息（含启停 + 网盘限制）
function getAllSources(cfg) {
  var enabled = getEnabledSet(cfg);
  return Object.keys(REGISTRY).map(function (id) {
    var m = REGISTRY[id];
    return { id: id, name: m.name, short: m.short, type: m.type, desc: m.desc, enabled: !!enabled[id], disks: getSourceDisks(id, cfg) };
  });
}
...
module.exports = { REGISTRY, parseSourceCfg, getEnabledSet, getSourceDisks, getAllSources, resolveSources, loadAdapter };
